...
do
	echo $line | grep "Shutdown complete"
		...
		PID=$(pgrep -f 'jboss.Main -c campus2')
		if [ -z "$PID" ]
		then
			${SCRIPT_PATH}/Limpiar_tmp_work_Nodo2.sh
			...
			JAVA_OPTS="${JAVA_OPTS} -Dcom.sun.management.jmxremote.port=22345 -Dcom.sun.management.jmxremote.authenticate=false"
